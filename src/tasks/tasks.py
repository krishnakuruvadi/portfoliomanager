...
def pull_and_store_coin_historical_vals(symbol, dt):
    ...
    for yr in range(dt.year, today.year+1):
        for month in range(1,12):
            ...
            try:
                HistoricalCoinPrice.objects.get(coin=coin, date=cdt)
                print(f'Crypto history from {coin} on {cdt} was found in the database and will not be created again.')
                exists += 1
            except HistoricalCoinPrice.DoesNotExist:
                print(f'Crypto history from {coin} on {cdt} was not found in the database. Attempting to retrieve price value.')
                val = get_historical_price(symbol, cdt)
                try:
                    HistoricalCoinPrice.objects.create(coin=coin, date=cdt, price=val) 
                    print(f'Crypto history {coin} {cdt} {val} was saved to the database.')       
                    added += 1
                except Exception as e:
                    print(f'Error {e} occurred when attempting to save {coin} {cdt} {val} to the database.')
            except Exception as e:
                print(f'Error {e} occured while getting crypto history for {coin} on {cdt} from the database.')

        yed = datetime.date(year=yr, month=12, day=31)
        if yed < datetime.date.today():
            try:
                HistoricalCoinPrice.objects.get(coin=coin, date=yed)
                print(f'Crypto history from {coin} on {yed} was found in the database and will not be created again.')
                exists += 1
            except HistoricalCoinPrice.DoesNotExist:
                print(f'Crypto history from {coin} on {yed} was not found in the database. Attempting to retrieve price value.')
                val = get_historical_price(symbol, yed)
                try:
                    HistoricalCoinPrice.objects.create(coin=coin, date=yed, price=val) 
                    print(f'Crypto history {coin} {yed} {val} was saved to the database.')       
                    added += 1
                except Exception as e:
                    print(f'Error {e} occurred when attempting to save {coin} {yed} {val} to the database.')
            except Exception as e:
                print(f'Error {e} occured while getting crypto history for {coin} on {yed} from the database.')
